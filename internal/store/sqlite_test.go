...
func TestLogToActiveWorkstream(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "test.db")
	s, _ := New(dbPath)
	defer s.Close()

	// Create workstreams with different states
	s.Create(&workstream.Workstream{Name: "done-ws", Project: "proj", State: workstream.StateDone})
	s.Create(&workstream.Workstream{Name: "active-ws", Project: "proj", State: workstream.StateInProgress})
	s.Create(&workstream.Workstream{Name: "pending-ws", Project: "proj", State: workstream.StatePending})

	// Log to active workstream
	wsName, err := s.LogToActiveWorkstream("proj", "Task completed: auth")
	if err != nil {
		t.Fatalf("LogToActiveWorkstream() error = %v", err)
	}
	if wsName != "active-ws" {
		t.Errorf("LogToActiveWorkstream() logged to %q, want 'active-ws'", wsName)
	}

	// Verify the log was added
	ws, _ := s.Get("proj", "active-ws")
	if len(ws.Log) != 1 {
		t.Fatalf("Log length = %d, want 1", len(ws.Log))
	}
	if ws.Log[0].Content != "Task completed: auth" {
		t.Errorf("Log content = %q, want 'Task completed: auth'", ws.Log[0].Content)
	}
}

func TestLogToActiveWorkstream_NoActive(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "test.db")
	s, _ := New(dbPath)
	defer s.Close()

	// Only done workstreams
	s.Create(&workstream.Workstream{Name: "done-ws", Project: "proj", State: workstream.StateDone})

	_, err := s.LogToActiveWorkstream("proj", "Task completed")
	if err == nil {
		t.Error("LogToActiveWorkstream() should error when no active workstream")
	}
}
